---
layout: post
title: "New Featured Artist: jeko"
date: 2026-04-18 19:00:00 +0000
series: featured_artists
---

An Australian origin isn't the only thing osu! and our newest Featured Artist have in common.

![](https://assets.ppy.sh/artists/538/header.jpg)

**jeko** (known around here as [J3C](https://osu.ppy.sh/users/15956540)) has been clicking circles for the better half of a decade, where they've been hearing the rhythm game-y sounds of renowned producers like [*Camellia*](https://osu.ppy.sh/beatmaps/artists/31), *celtix* and *ReeK*. 

Consistent exposure is bound to have an effect. In the words of **jeko**...

> osu! is the biggest cause for my music style

Through scattered tournament songs, presence in the Ranked section, and official inclusion in our Featured Artist catalogue as of today, **jeko**'s osu!-inspired sound is now ready to inspire the rhythm game soundscape's next generation.

To be more specific, **17** tracks are officially part of [**jeko**'s Featured Artist listing](https://osu.ppy.sh/beatmaps/artists/538)! That means at a moment's notice, you can pick up a pre-timed `.osz` and make something cool (or just *something* if you've never opened the editor before).

Regardless of how you engage with **jeko**'s music, we can all agree that it's awesome. Listen to some previews below if you haven't already:

### jeko - Geheimnis

While **jeko** is a circle clicker, their music's introduction to the wider osu! audience was [this 4K osu!mania beatmap](https://osu.ppy.sh/beatmapsets/2463674) hosted by [FAMoss](https://osu.ppy.sh/users/7707789)!

<div align="center" class="osu-md__paragraph">
    <video width="95%" controls>
        <source src="https://assets.ppy.sh/media/news/jeko_release_showcase.mp4" type="video/mp4" preload="none">
    </video>
</div>

### jeko - persistence

A technical song calls for [a technical map](https://osu.ppy.sh/beatmapsets/2494074). Thank [Roupus](https://osu.ppy.sh/users/11117670) and [virtuoso](https://osu.ppy.sh/users/7540631) for delivering.

<audio controls>
    <source src="https://assets.ppy.sh/artists/538/Songs/jeko%20-%20persistence.mp3">
</audio>

### jeko - Magnitude

<audio controls>
    <source src="https://assets.ppy.sh/artists/538/Songs/jeko%20-%20Magnitude.mp3">
</audio>

### jeko - Etheria

<audio controls>
    <source src="https://assets.ppy.sh/artists/538/Songs/jeko%20-%20Etheria.mp3">
</audio>

### jeko - ULTRACRITICAL \[PROVE YOURSELF\]

<audio controls>
    <source src="https://assets.ppy.sh/artists/538/Songs/jeko%20-%20ULTRACRITICAL%20%5BPROVE%20YOURSELF%5D.mp3">
</audio>

---

The 5 songs above are just a taste of what's on [**jeko**'s Featured Artist listing](https://osu.ppy.sh/beatmaps/artists/538). You shouldn't need any convincing by this point.

Entertain this idea for a second. You're a mapper and you're looking for interesting music to map. What do you do? Do you make a Nightcore version of a song that you somehow associate with the colour pink? Maybe, but here's an alternative: wait until next weekend for a new Featured Artist!

Stay tuned.

—pishifat
