...
- **The following technical limitations or conventions are put in place to ensure consistency:**
  ...
  - **If the artist or title field exceeds the uploadable maximum character length of 81, or both together cause Windows filenames for the `.osu` files to exceed 255 characters, any additional markers from the fields causing this have to be dropped consistently and if this is still not sufficient, the corresponding fields need to be abbreviated reasonably and end in `...` to signal that this song title has been shortened.** This would mainly apply to the following template linking to a .osu file:
  ``` 
  C:\Users\USERNAME\AppData\Local\osu!\Songs\SETID Artist - Title\Artist - Title (HOST) [DIFF].osu 
  ```
...
- **The following conventions are enforced in order to standardise the general handling of metadata:**
  ...
  - **If a track has 2 individual tracks compined, they must use `/` or `\` as a seperator between each track name in the order shown.**
...
- **Romanisation of characters and languages other than English are to be handled the following ways:**
  ...
  - **Songs with German metadata must romanise umlauts into two-letter equivalents (`ue`, `oe`, `ae` and `ss`).** Unless the umlauts is used for design or theming purposes which is through a case-by-case basis.
...
-   **Avoid using storyboarded hit sounds.** If a player misses the hit object that the storyboarded hit sound lands on, then the hit sound will be played (which doesn't make much sense when this is a rhythm game utilizing audio feedback). Also, storyboarded hit-sounds at places where there are no objects can confuse the player into thinking there was some unseen hit object, which does not make sense either. e.g. [Hatsune Miku - Test Tube Princess](https://osu.ppy.sh/s/6907)
